...
- (void)initCrashReport {
    ...
    if ([version count] >= 3) {
        if ([[version objectAtIndex:0] intValue] > 0 && [[version objectAtIndex:1] intValue]> 0) {
            //启动bugly组件，bugly组件为腾讯提供的用于crash上报和分析的开放组件，如果您不需要该组件，可以自行移除
            BuglyConfig * config = [[BuglyConfig alloc] init];
            NSString* ver = [NSString stringWithFormat:@"%@.%@.%@",[version objectAtIndex:0],[version objectAtIndex:1],[version objectAtIndex:2]];
            config.version = ver;
#if DEBUG
