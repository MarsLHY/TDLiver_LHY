...
//设置不同hub提示画面
- (void)showHub:(TDHubModel*)hubModel {
    if (!hubModel) {
        return;
    }
    if (hubModel.hubType == TDHubDefalut) {
        [SVProgressHUD showWithStatus:hubModel.title];
    }
}


- (void)dismissHub:(TDHubModel*)hubModel {
    if (!hubModel) {
        return;
    }
    if (hubModel.hubType == TDHubDefalut) {
