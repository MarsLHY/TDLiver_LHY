...
@interface TCPushController () <AVCaptureVideoDataOutputSampleBufferDelegate, MCCameraDynamicDelegate>
...
@implementation TCPushController
{
    BOOL _camera_switch;
    float  _beauty_level;
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (EvtID >= 0) {
            ...
            else if (EvtID == PUSH_EVT_PUSH_BEGIN)
            {
                ...
                [[TCPusherMgr sharedInstance] changeLiveStatus:_liveInfo.userid status:TCLiveStatus_Online handler:^(int errCode) {
