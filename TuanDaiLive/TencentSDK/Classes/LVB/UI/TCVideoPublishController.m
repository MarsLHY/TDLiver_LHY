...
- (instancetype)init:(id)videoRecorder recordType:(NSInteger)recordType RecordResult:(TXRecordResult *)recordResult  TCLiveInfo:(TCLiveInfo *)liveInfo
{
    ...
    if (self) {
        ...
        _videoPublishParams.secretId = kTCCloudAPISecretId;
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    _playEnable = YES;
    if (_isPublished == NO && !_isPublishing) {
...
{
    //限制下上传文件的大小
    TXVideoInfo *videoInfo = [TXUGCVideoInfoReader getVideoInfo:_recordResult.videoPath];
    if (videoInfo.fileSize >= 50 * 1024 * 1024) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"文件大小超过50M,暂不支持上传！" message:@"" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:@"", nil];
        [alertView show];
        return;
    }
...
//    NSDictionary* dictParam = @{@"Action" : @"GetVodSignature"};
    ...
    [TCUtil asyncSendHttpRequest:dictParam handler:^(int result, NSDictionary *resultDict) {
        ...
        if (resultDict)
        {
            ...
            if (0 == errCode && resultDict[@"signature"]) {
                _videoPublishParams.signature  = resultDict[@"signature"];
                _videoPublishParams.coverImage = _recordResult.coverImage;
        ...
        if (errCode) {
            [self toastTip:[NSString stringWithFormat:@"无法发布[errcode:%d]", errCode]];
            return;
        }
...
- (void)textViewDidChange:(UITextView*)textView
{
    ...
    _labLeftWords.text = [NSString stringWithFormat:@"%02ld/500", 500 - [textView.text length]];
}
...
-(void) onPublishComplete:(TXPublishResult*)result
{
    ...
    NSDictionary* userInfo = @{@"nickname" : profile.nickName, @"headpic" : profile.faceURL, @"frontcover":result.coverURL == nil ? @"" : result.coverURL, @"location":@"未知"};
    NSDictionary* dictParam = @{@"Action" : @"UploadUGCVideo", @"userid":profile.identifier, @"file_id":result.videoId, @"title":title, @"play_url":result.videoURL, @"userinfo":userInfo};
    [TCUtil asyncSendHttpRequest:dictParam handler:^(int rst, NSDictionary *resultDict) {
        if (0 == rst) {
            if (_selectBtnTag >= 0) {
            ...
            [self toastTip:[NSString stringWithFormat:@"UploadUGCVideo Failed[%d]", rst]];
        }
...
- (void)publishFinished
{
    ...
    if ([_videoRecorder isMemberOfClass:[TXLivePlayer class]]) {
        ...
        [self dismissViewControllerAnimated:YES completion:^{
            //界面显示到小视频列表(针对小直播APP做，该模块给其它用时应该去掉以下)
            UITabBarController* tabController = (UITabBarController*)((AppDelegate*)[UIApplication sharedApplication].delegate).window.rootViewController;
            TCLiveListViewController* listViewController = (TCLiveListViewController*)tabController.viewControllers[0].childViewControllers[0];
            [listViewController showUGCVideoList];
