...
//    [[TCIMPlatform sharedInstance] setGroupAssistantListener:self];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1) {
        TCUserInfoData  *userInfoData = [[TCUserInfoMgr sharedInstance] getUserProfile ];
        [[TCPlayerMgr sharedInstance] reportUser:_liveInfo.userid hostUserId:userInfoData.identifier handler:^(int errCode) {
            //todo
        }];
...
-(void)clickPraise:(UIButton *)button{
    ...
    if ([_msgHandler sendLikeMessage:profile.identifier nickName:profile.nickName headPic:profile.faceURL]) {
        [[TCPlayerMgr sharedInstance] giveLike:_liveInfo.userid handler:^(int errCode) {
        }];
