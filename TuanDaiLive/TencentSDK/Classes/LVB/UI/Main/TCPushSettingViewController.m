...
#endif
#import "TCLinkMicPushController.h"
...
@implementation TCPushSettingViewController {
...
-(void)keyboardFrameDidChange:(NSNotification*)notice
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        if (endFrame.origin.y == self.view.height) {
            _titleTextViewTopConstraint.constant =   3;
        }else{
            if (endFrame.origin.y - _titleTextView.height < _titleTextView.y) {
                _titleTextViewTopConstraint.constant +=  endFrame.origin.y - _titleTextView.height - _titleTextView.y;
            }
        }
    }];
}
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self.publishBtn setBackgroundColor:[UIColor colorWithRed:36/255.0 green:203/255.0 blue:173/255.0 alpha:1]];
    [self.publishBtn setFrame:CGRectMake(0, self.view.bottom - 55, self.view.width, 55)];
...
- (void)uploadImage {
    ...
    if (self.selectedCoverImage) {
        [[TCUploadHelper shareInstance] upload:profile.identifier image:self.selectedCoverImage completion:^(int errCode, NSString *imageSaveUrl) {
            ...
            [[TCUserInfoMgr sharedInstance] saveUserCover:imageSaveUrl handler:^(int errCode, NSString *strMsg) {
                if (errCode != 0)
                {
...
- (void)startPush
{
    [[TCIMPlatform sharedInstance] reLogin:^{
        ...
        NSString *userId = [[TCIMPlatform sharedInstance] getLoginParam].identifier;
...
//#if POD_PITU
//    TCPublishControllerEx *pubVC = [[TCPublishControllerEx alloc] initWithPublishInfo:_liveInfo];
//#else
    TCLinkMicPushController *pubVC = [[TCLinkMicPushController alloc] initWithPublishInfo:_liveInfo];
//#endif

    NSString *str = _selectShare.titleLabel.text;
    if ([str equalsString:@"QQ"]) {
        pubVC.platformType = UMSocialPlatformType_QQ;
    }
    else if ([str equalsString:@"Qzone"]) {
        pubVC.platformType = UMSocialPlatformType_Qzone;
    }
    else if ([str equalsString:@"WechatTimeLine"]) {
        pubVC.platformType = UMSocialPlatformType_WechatTimeLine;
    }
    else if ([str equalsString:@"WechatSession"]) {
        pubVC.platformType = UMSocialPlatformType_WechatSession;
    }
    else if ([str equalsString:@"Sina"]) {
        pubVC.platformType = UMSocialPlatformType_Sina;
    }
