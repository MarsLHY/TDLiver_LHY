...
@interface TCLinkMicPlayController()<ITCLivePushListener, ITCLivePlayListener>
{
...
@implementation TCLinkMicPlayController
...
-(id)initWithPlayInfo:(TCLiveInfo *)info  videoIsReady:(videoIsReadyBlock)videoIsReady {
    ...
    _tcLinkMicMgr = [TCLinkMicMgr sharedInstance];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if (_liveInfo.type == TCLiveListItemType_Live) {
        ...
        //初始化连麦播放小窗口里的logView
        for (TCLinkMicPlayItem * item in _playItems) {
            if (item.logView == nil) {
...
- (void) initPlayItem: (int)index {
    ...
    TCLinkMicPlayItem* playItem = [[TCLinkMicPlayItem alloc] init];
...
-(void)stopLinkMic {
    ...
    //结束拉流
    for (TCLinkMicPlayItem* playItem in _playItems) {
...
-(void) onReceiveLinkMicResponse:(NSString*)userID withType:(TCLinkMicResponseType)rspType andParam:(NSDictionary*)param {
    ...
    if (LINKMIC_RESPONSE_TYPE_ACCEPT == rspType) {
        ...
        [[TCPusherMgr sharedInstance] getPushUrlForLinkMic:profile.identifier title:@"连麦" coverPic: profile.coverURL nickName:profile.nickName headPic:profile.faceURL location:@"" handler:^(int errCode, NSString* pusherUrl, NSInteger timestamp, NSString* playUrl) {
...
-(void)onLivePushEvent:(NSString*) pushUrl withEvtID:(int)event andParam:(NSDictionary*)param {
    if (event == PUSH_EVT_PUSH_BEGIN) {             //开始推流事件通知
        //1.拉取主播的低时延流
        TCUserInfoData * profile = [[TCUserInfoMgr sharedInstance] getUserProfile];
        [[TCPlayerMgr sharedInstance] getPlayUrlWithSignature:profile.identifier originPlayUrl:_liveInfo.playurl handler:^(int errCode, NSString *playUrl) {
            if (errCode == 0 && playUrl != nil && playUrl.length > 0) {
                TCLinkMicPlayItem * playItem = [self getPlayItemByUserID:_liveInfo.userid];
                if (playItem) {
...
-(void)onLivePlayEvent:(NSString*) playUrl withEvtID:(int)event andParam:(NSDictionary*)param {
    TCLinkMicPlayItem * playItem = [self getPlayItemByStreamUrl:playUrl];
    if (playItem == nil) {
        return;
    }
...
-(TCLinkMicPlayItem*) getPlayItemByUserID:(NSString*)userID {
    if (userID) {
        for (TCLinkMicPlayItem* playItem in _playItems) {
...
-(TCLinkMicPlayItem*) getPlayItemByStreamUrl:(NSString*)streamUrl {
    if (streamUrl) {
        for (TCLinkMicPlayItem* playItem in _playItems) {
...
-(void) stopPlayVideoStream: (NSString*)userID {
    TCLinkMicPlayItem * playItem = [self getPlayItemByUserID:userID];
    if (playItem) {
        [playItem stopLoading];
        [playItem stopPlay];
        [playItem emptyPlayInfo];
...
-(TCPlayDecorateView*) findPlayDecorateView {
    for (id view in self.view.subviews) {
        if ([view isKindOfClass:[TCPlayDecorateView class]]) {
            return (TCPlayDecorateView*)view;
        }
    }
    return nil;
}
