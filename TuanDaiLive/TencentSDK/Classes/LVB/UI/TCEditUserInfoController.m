...
@implementation TCEditUserInfoController
...
{
    TIMGender dic = TIM_GENDER_FEMALE;
    ...
    {
        [[TCUserInfoMgr sharedInstance] saveUserGender:dic handler:^(int code, NSString *msg)
         {
    ...
    [[TCUploadHelper shareInstance] upload:profile.identifier image:image completion:^(int errCode, NSString *imageSaveUrl)
     {
         ...
         {
             [[TCUserInfoMgr sharedInstance] saveUserFace:imageSaveUrl handler:^(int code, NSString *msg)
              {
                  if (ERROR_SUCESS != errCode)
                  {
...
- (void)uploadUserInfoNick:(TCEditUserInfoTableViewCell*)cell
{
    ...
    {
        [[TCUserInfoMgr sharedInstance] saveUserNickName:cell->nickText.text handler:^(int code, NSString *msg)
         {
             if (ERROR_SUCESS != code)
             {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    UIImage *cutImage = [TCEditUserInfoController cutImage:image];
    [picker dismissViewControllerAnimated:YES completion:nil];
...
+ (UIImage *)cutImage:(UIImage *)image
{
    ...
    if (image)
    {
        ...
        {
            // 直接上传
            image = [TCEditUserInfoController scaleToSize:image size:pubSize];
            return image;
        }
        else
        {
            if (imgRatio > 1)
            {
                ...
                UIImage *upimg = [TCEditUserInfoController cropImage:image inRect:CGRectMake(0, (image.size.height - upSize.height)/2, upSize.width, upSize.height)];
                upimg = [TCEditUserInfoController scaleToSize:upimg size:pubSize];
                return upimg;
            }
            else
            {
                ...
                UIImage *upimg = [TCEditUserInfoController cropImage:image inRect:CGRectMake((image.size.width - upSize.width)/2, 0, upSize.width, upSize.height)];
                upimg = [TCEditUserInfoController scaleToSize:upimg size:pubSize];
                return upimg;
            }
        }
