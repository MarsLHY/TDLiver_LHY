...
@interface TCLinkMicPushController()<ITCLivePlayListener>
{
    NSString*               _sessionId;
...
@implementation TCLinkMicPushController
...
-(void) onReceiveMemberJoinNotify:(NSString*)userID withPlayUrl:(NSString*)strPlayUrl {
    ...
    TCLinkMicPlayItem * item = [self getPlayItemByUserID:userID];
    if (item == nil) {
        return;
    }
    ...
    [[TCPlayerMgr sharedInstance] getPlayUrlWithSignature:profile.identifier originPlayUrl:strPlayUrl handler:^(int errCode, NSString *playUrl) {
...
-(void)onLivePlayEvent:(NSString*) strStreamUrl withEvtID:(int)event andParam:(NSDictionary*)param {
    TCLinkMicPlayItem * playItem = [self getPlayItemByStreamUrl:strStreamUrl];
    if (playItem == nil) {
        return;
    }
...
- (NSString*) getLinkMicSessionID {
    ...
    //NSString* strSessionID = [TCLinkMicMgr getStreamIDByStreamUrl:self.rtmpUrl];
