...
-(void)getGroupMembers{
    ...
    [[TCPlayerMgr sharedInstance] fetchGroupMemberList:_liveInfo.userid groupId:realGroupId handler:^(int errCode, int memberCount, NSArray *memberList) {
        if (0 == errCode)
        {
            ...
            if (memberList)
            {
                ...
                {
                    TCUserInfoData *data = [[TCUserInfoData alloc] init];
                    data.nickName = memInfo.nickname;
                    data.identifier = memInfo.userid;
                    data.faceURL = memInfo.headpic;
                    [_dataArray addObject:data];
                }
            }
            [self reloadData];
            
            if (self.audienceListDelegate)
                [self.audienceListDelegate onFetchGroupMemberList:errCode memberCount:memberCount];
        }
    }];
}
